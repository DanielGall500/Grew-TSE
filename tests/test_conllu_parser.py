...
@pytest.fixture
def get_parser(get_test_set_path, get_test_constraints):
    parser = ConlluParser()
    parser.parse(get_test_set_path, get_test_constraints)
    return parser


def test_build_lexical_item_set(get_parser, get_test_set_path):
...
def test_build_masked_dataset(get_parser, get_test_set_path):
    constraints = {"Mood": "Sub"}
    results = get_parser._build_masked_dataset(get_test_set_path, constraints, "[MASK]")
...
def test_candidate_set(get_parser):
