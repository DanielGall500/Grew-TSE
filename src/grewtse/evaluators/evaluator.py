...
from typing import Tuple, NamedTuple, List, Any, Literal
...
class GrewTSEvaluator:
    ...
    def evaluate_model(
        ...
        model_repo: str,
        task_type: Literal[Task],  # can be 'mlm' or 'ntp'
        ...
        with self.evaluator.load_model(model_repo, is_mlm, device):
            ...
            for row in tqdm(mp_dataset_iter, total=n, desc="Evaluating"):
                ...
                try:
                    if is_mlm:
                        self._evaluate_row_mlm(row, row_results, evaluation_cols)
                    else:
                        self._evaluate_row_ntp(row, row_results, evaluation_cols)
    ...
    def evaluate_from_filepath(
        ...
        return self.evaluate_model(
            mp_dataset, model_repo, task_type, evaluation_cols, entropy_topk, save_to, device, row_limit
        )
    ...
    def _evaluate_row_mlm(self, row, row_results, evaluation_cols:list[str]=["form_grammatical","form_ungrammatical"]):
        try:
    ...
    def _evaluate_row_ntp(self, row, row_results, evaluation_cols:list[str]=["form_grammatical","form_ungrammatical"]):
        try:
            targets = [getattr(row,c) for c in evaluation_cols]
    ...
    def get_avg_surprisal_difference(self, grammatical_column:str="p_form_grammatical", ungrammatical_column:str="p_form_ungrammatical") -> float:
        """
    ...
    def get_norm_avg_surprisal_difference(self, grammatical_column:str="p_form_grammatical",ungrammatical_column:str="p_form_ungrammatical") -> float:
        """
        Get the normalised average surprisal difference (ASD).
        A higher score indicates that the model, on average, tends towards being more confident in the grammatical word over the ungrammatical one.
        However, this is not quite fully accurate and as with any average ASD scores may suffer from outliers skewing the result.
    ...
    def get_accuracy(self, grammatical_column:str="p_form_grammatical",ungrammatical_column:str="p_form_ungrammatical") -> float:
        """
    ...
    def _get_grammatical_form_probs(self, col:str="p_form_grammatical") -> pd.Series|None:
    ...
    def _get_ungrammatical_form_probs(self, col:str="p_form_ungrammatical") -> pd.Series|None:
        if self.evaluation_dataset is not None and col in self.evaluation_dataset.columns:
