...
def find_sentence_mask_index(full_sentence: str, token_list: list[str], token_list_mask_index: str, skippable_tokens: list[str]):
    ...
    # RECURSIVE CASE
        ...
        print("Match: ", t)
        print(f"Adding {match_index} characters in between and {len(t)}, remainder: -{sliced_sentence}-")
        
        return match_index + len(t) + find_sentence_mask_index(sliced_sentence, token_list, token_list_mask_index-1, skippable_tokens)
    else:
